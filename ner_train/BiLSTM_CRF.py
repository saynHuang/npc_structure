...
def train_model(data_dir, model_save_dir):
    ...
    # BiLSTM_CRF_layer
    lstm_fw_cell = tf.nn.rnn_cell.LSTMCell(n_units, forget_bias=1.0, state_is_tuple=True)  # 前向LSTM
    lstm_bw_cell = tf.nn.rnn_cell.LSTMCell(n_units, forget_bias=1.0, state_is_tuple=True)  # 后向LSTM
    ...
    bilstm_out = tf.concat([output_fw, output_bw], axis=2, name='bilstm_out')  # [n_samples, seq_len, 2*n_units]
    logit = tf.reshape(bilstm_out, [-1, 2*n_units])

    # Fully connected layer.
    with tf.name_scope('fully_connect'):
        W = tf.get_variable(name='fc_W', shape=[2*n_units, n_units], dtype=tf.float32)
        b = tf.get_variable(name='fc_b', shape=[n_units], dtype=tf.float32, initializer=tf.zeros_initializer())
        predict = tf.tanh(tf.matmul(logit, W) + b)  # [n_samples*seq_len, n_units]
    ...
    with tf.Session() as sess:
        ...
        try:
            while not coord.should_stop():
                ...
                if n_samples//(n_batch*batch_size) == 0:
                    ...
                    epoch += 1
                    # if epoch % 10 == 0:
                    print('epoch %3d: Loss --> train:%.4f\t dev:%.4f' % (epoch, tr_loss, de_loss))
        ...
        save_path = saver.save(sess, os.path.join(model_save_dir, 'bilstm_crf'))
        print("Model saved in file: %s" % save_path)

        transition_params = sess.run(transition_params,
                                     feed_dict={xs: x_data, ys: y_data, sequence_lengths: efficient_seq_len})
        with open(os.path.join(model_save_dir, 'trans_matrix.pkl'), 'wb') as trans_mat:
            pickle.dump(transition_params, trans_mat)

    plot_loss(train_loss, dev_loss, model_save_dir, 'bilstm_crf')
...
def load_model(data_dir, model_save_dir):
    ...
    model_name = 'bilstm_crf.meta'
    pred_name = 'bilstm_crf_predict_score.txt'
    # 加载转移矩阵
...
if __name__ == '__main__':
    ...
    epochs = 50
    lr = 0.001
    model_save_dir = '../ner_train/BiLSTM_CRF_Model'
