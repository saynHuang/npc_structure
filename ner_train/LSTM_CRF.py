...
def train_model(data_dir, model_save_dir):
    ...
    # LSTM_CRF_layer
    lstm_cell = tf.nn.rnn_cell.LSTMCell(n_units, forget_bias=1.0, state_is_tuple=True)
    output, states = tf.nn.dynamic_rnn(lstm_cell, input_embedded, dtype=tf.float32)

    logit = tf.reshape(output, [-1, n_units])

    # Fully connected layer.
    with tf.name_scope('fully_connect'):
        W = tf.get_variable(name='fc_W', shape=[n_units, n_units], dtype=tf.float32)
    ...
    with tf.Session() as sess:
        ...
        save_path = saver.save(sess, os.path.join(model_save_dir, 'lstm_crf'))
    ...
    plot_loss(train_loss, dev_loss, model_save_dir, 'lstm_crf')
...
def load_model(data_dir, model_save_dir):
    ...
    model_name = 'lstm_crf.meta'
    pred_name = 'lstm_crf_predict_score.txt'
    # 加载转移矩阵
...
if __name__ == '__main__':
    ...
    epochs = 50
    lr = 0.001
    model_save_dir = '../ner_train/LSTM_CRF_Model'
